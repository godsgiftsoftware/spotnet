...
use super::utils::{
    setup_test_suite, deploy_erc20_mock_2, get_risk_factor, deploy_pragma_mock,
    calculate_health_factor, store_data_for_health_factor,
};
...
fn test_set_risk_less_than() {
    ...
    let suite = setup_test_suite(owner, deploy_erc20_mock_2(), deploy_pragma_mock(), false);
...
fn test_set_risk_ok() {
    ...
    let risk_factor_from_storage = get_risk_factor(
...
fn test_get_health_factor() {
    ...
    stop_cheat_caller_address(suite.margin.contract_address);
    println!("HF Act: {health_factor}");
    println!("HF Calc: {}", calculate_health_factor(@suite, risk_factor));
    assert(
        health_factor == calculate_health_factor(@suite, risk_factor), 'Health factor incorrect',
    );
}
